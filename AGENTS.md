...
This directory is a local mirror of the ChatGPT project “UniversalCRM”.
...
- `config.js` — sets `window.DASHBOARD_CONFIG`: GitHub org/owner, the four repo names (`product`, `operations`, `sales`, `ideas`), the GitHub Project number, issue label names, and the `team` roster. This is the single file a new deployment must edit.
- `app.js` — one IIFE, no modules, no dependencies. It:
  ...
  - Handles UI interactions (`setupInteractions`): the idea-submission dialog (builds a prefilled GitHub "new issue" URL — never posts on the user's behalf), workstream tab switching, sidebar active-link state.
